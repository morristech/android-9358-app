...
public class CustomKeyboardView extends View implements View.OnClickListener {
    ...
    private Map<String, Integer> mKeyTextColorMap = new HashMap<>();
    private Map<String, Drawable> mKeyBackgroundDrawableMap = new HashMap<>();
    private Map<String, String> mKeyLableMap = new HashMap<>();
    ...
    /**
     * Listener for {@link OnMyKeyboardCallback}.
     */
    private OnMyKeyboardCallback mKeyboardActionListener;
    ...
    public CustomKeyboardView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a =
                context.obtainStyledAttributes(
                        attrs, R.styleable.CustomKeyboardView, defStyle, 0);
...
//                android.internal.R.bool.config_swipeDisambiguation);
...
//        mAccessibilityManager = AccessibilityManager.getInstance(context);
    ...
    public void setOnKeyboardActionListener(OnMyKeyboardCallback listener) {
        mKeyboardActionListener = listener;
    }

    /**
     * Returns the {@link OnMyKeyboardCallback} object.
     *
     * @return the listener attached to this keyboard
     */
    protected OnMyKeyboardCallback getOnKeyboardActionListener() {
    ...
    /**
     * When enabled, calls to {@link OnMyKeyboardCallback#onKey} will include key
...
//        final Drawable keyBackground = mKeyBackground;
        ...
        for (int i = 0; i < keyCount; i++) {
            ...
            label = getRealLable(label);

            int[] drawableState = key.getCurrentDrawableState();
            Drawable keyBackground = getKeyBackground(label);
            ...
            if (key.icon != null) {
                ...
                paint.setColor(getKeyTextColor(label));
...
//                }
//                text = getContext().getString(R.string.keyboard_headset_required_to_hear_password);
//            } else {
//                text = getContext().getString(R.string.keyboard_password_character_no_headset);
//            }
...
//                    final int touchX = (int) event.getX() - getPaddingLeft();
...
//        }
    ...
    /**
     * @param key
     * @param color format: 0xAARRGGBB
     */
    public void setKeyTextColor(String key, int color) {
        mKeyTextColorMap.put(key, color);
    }

    public int getKeyTextColor(String key) {
        Integer color = mKeyTextColorMap.get(key);
        if (color != null) {
            return color;
        } else {
    ...
    public void setKeyBackgroundDrawable(String key, Drawable drawable) {
        mKeyBackgroundDrawableMap.put(key, drawable);
    }

    public Drawable getKeyBackground(String key) {
        Drawable drawable = mKeyBackgroundDrawableMap.get(key);
        if (drawable != null) {
            return drawable;
        } else {
    ...
    public String getRealLable(String keyLable) {
        String newLable = mKeyLableMap.get(keyLable);
        return newLable == null ? keyLable : newLable;
    }

    public void setKeyLableMap(String oldKeyLable, String newKeyLable) {
        mKeyLableMap.put(oldKeyLable, newKeyLable);
    }
}