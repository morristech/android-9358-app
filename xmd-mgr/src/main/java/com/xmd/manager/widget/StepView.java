...
public class StepView extends View {
    ...
    public StepView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTodoTextColor = a.getColor(R.styleable.StepView_todoTextColor, Color.GRAY);
        mDoneTextColor = a.getColor(R.styleable.StepView_doneTextColor, Color.RED);
        ...
        mTodoTextPaint = new Paint();
        mTodoTextPaint.setColor(mTodoTextColor);
        mTodoTextPaint.setTextSize(mTextSize);
        mTodoTextPaint.setAntiAlias(true);
        mTodoTextPaint.setStrokeWidth(1);
        mTodoTextPaint.setFlags(Paint.ANTI_ALIAS_FLAG);

        mDoneTextPaint = new Paint();
        mDoneTextPaint.setColor(mDoneTextColor);
        mDoneTextPaint.setTextSize(mTextSize);
        mDoneTextPaint.setAntiAlias(true);
        mDoneTextPaint.setStrokeWidth(1);
        mDoneTextPaint.setFlags(Paint.ANTI_ALIAS_FLAG);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mTotolStep == 1) {
            ...
            if (withDesc) {
                ...
                canvas.drawText(text, textStartX, textY, mCurrentStep >= 0 ? mDoneTextPaint : mTodoTextPaint);
            }
        } else {
            if (withDesc) {
