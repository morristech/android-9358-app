 ...
 */
public class WaitingReturnVisitFragment extends BaseListFragment<BadComment> {

    private Subscription mGetWaitingReturnVisitCommentsSubscription;
    private Subscription mBadCommentStatusSubscription;
    private View view;
    private String callPhone;
    private static final int REQUEST_CODE_PHONE = 0x0077;

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_wait_return_visit, container, false);
    ...
    private void handleBadCommentList(BadCommentListResult badComment) {
        ...
        if (badComment.statusCode == 200 && badComment.commentState.equals(RequestConstant.VALID_COMMENT)) {
        ...
        BottomPopupWindow popupWindow = BottomPopupWindow.getInstance(getActivity(), phone, emChatId, commentId, returnStatus, new BottomPopupWindow.OnRootSelectedListener() {
            ...
            public void onItemSelected(ReturnVisitMenu rootMenu) {
                switch (rootMenu.getType()) {
                    case 1:
                        PermissionTool.requestPermission(WaitingReturnVisitFragment.this, new String[]{Manifest.permission.CALL_PHONE}, new String[]{"拨打电话"}, REQUEST_CODE_PHONE);
                        break;
                    case 2:
